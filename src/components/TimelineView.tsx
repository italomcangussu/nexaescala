...
const TimelineView: React.FC<TimelineViewProps> = ({ logs }) => {
  const getIcon = (type: string) => {
    switch (type) {
      case 'SHIFT_SWAP': return <ArrowRightLeft size={16} className="text-primary dark:text-primary-light" />;
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold text-primary dark:text-primary-light mb-6">Atividade do Serviço</h2>
      
      <div className="relative border-l-2 border-slate-100 dark:border-slate-800 ml-4 space-y-8 transition-colors">
        {logs.map(log => (
          <div key={log.id} className="relative pl-8 group">
            {/* Connector Dot */}
            <div className="absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-white dark:bg-surface-dark border-2 border-slate-200 dark:border-slate-700 group-hover:border-primary dark:group-hover:border-primary-light transition-colors flex items-center justify-center">
               <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600 group-hover:bg-primary dark:group-hover:bg-primary-light transition-colors"></div>
            </div>

            {/* Content Card */}
            <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start space-x-4 transition-colors">
              <img 
              ...
              <div className="flex-1">
                ...
                <div className="inline-flex items-center space-x-2 px-2 py-1 bg-surface dark:bg-slate-800 rounded-md">
