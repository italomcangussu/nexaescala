...
const ProfileView: React.FC<ProfileViewProps> = ({ profile, currentUser, onBack, onEdit }) => {
   ...
   return (
      <div className="fixed inset-0 bg-background dark:bg-background-dark z-60 overflow-y-auto flex flex-col animate-fade-in-up transition-colors duration-300">

         {/* Header Image - Taller and safe area aware */}
         <div className="h-48 bg-linear-to-r from-emerald-600 to-teal-500 relative shrink-0">
            <div className="absolute top-0 left-0 w-full pt-safe">
         ...
         <div className="flex-1 -mt-16 px-4 pb-20">
            <div className="bg-white dark:bg-surface-dark rounded-3xl shadow-lg border border-slate-100 dark:border-slate-800 p-6 pt-0 relative flex flex-col items-center transition-colors">

               {/* Avatar - Centered and overlapping */}
               <div className="-mt-16 mb-4 relative">
                  <div className="w-32 h-32 rounded-full p-1.5 bg-white dark:bg-surface-dark shadow-xl transition-colors">
                     <img src={profile.avatar_url} alt={profile.full_name} className="w-full h-full rounded-full object-cover" />
                  </div>
                  {isMe && (
                     <div className="absolute bottom-1 right-1 bg-primary text-white p-2 rounded-full border-4 border-white dark:border-slate-900 shadow-sm cursor-pointer hover:bg-primary-dark transition-colors" onClick={onEdit}>
                        <Edit2 size={14} />
                     </div>
                  )}
               </div>
               ...
               <div className="text-center w-full mb-6">
                  ...
                  <p className="text-base text-primary dark:text-primary-light font-semibold mt-1">{profile.specialty || 'Médico Generalista'}</p>
               ...
               <div className="w-full grid grid-cols-3 gap-2 py-4 border-t border-b border-slate-50 dark:border-slate-800 mb-6 transition-colors">
                  <div className="text-center p-2 rounded-xl hover:bg-surface dark:hover:bg-slate-800 transition-colors cursor-pointer">
               ...
               <div className="w-full">
                  {isMe ? (
                     ...
                     <button
                        ...
                        className={`w-full py-3.5 font-bold rounded-2xl transition-all flex items-center justify-center space-x-2 active:scale-95 ${isFollowing ? 'bg-white dark:bg-slate-800 border-2 border-primary text-primary dark:text-primary-light' : 'bg-primary text-white shadow-lg shadow-emerald-200 dark:shadow-none hover:shadow-emerald-300'}`}
                     >
                        {isFollowing ? (
                           <>
                              <Check size={20} />
...
const InfoCard = ({ icon, title, value }: { icon: React.ReactNode, title: string, value: string }) => (
   <div className="bg-white dark:bg-surface-dark p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center space-x-4 shadow-sm hover:shadow-md transition-all">
      <div className="w-12 h-12 rounded-full bg-surface dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 shrink-0">
         {icon}
      </div>
      <div>
