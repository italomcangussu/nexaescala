...
const ServiceWizard: React.FC<ServiceWizardProps> = ({ onClose, currentUser, onFinish }) => {
  ...
  // Helper to ensure shift setters work with the UI which might expect direct state access pattern
  // But we expose setShifts and setEditingShift directly from hook so it should be fine.
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center pointer-events-none">
      ...
      <div
        ...
        className="relative w-full max-w-lg mx-4 mb-4 sm:mb-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] sm:max-h-[80vh] transition-all animate-fade-in-up z-20 pointer-events-auto border border-white/20 dark:border-slate-700/50"
      >
        ...
        {showCompletion && (
          <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl p-8 relative shadow-2xl overflow-hidden flex flex-col items-center">
              {/* Confetti or decorative bg could go here */}

              <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400 mb-6 animate-bounce shadow-lg shadow-emerald-200 dark:shadow-none">
                <CheckCircle size={40} strokeWidth={3} />
              </div>

              <h2 className="text-2xl font-bold text-slate-800 dark:text-white text-center mb-2">Serviço Criado!</h2>
              <p className="text-slate-500 dark:text-slate-400 text-center mb-8 max-w-[250px] mx-auto text-sm leading-relaxed">
                O serviço <span className="font-bold text-slate-800 dark:text-slate-200">{serviceName}</span> foi configurado e salvo.
              </p>

              <div className="w-full space-y-3">
                <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-800/50 text-center mb-4">
                  <p className="font-bold text-blue-800 dark:text-blue-300 text-sm">Próximo passo: Montar a Escala</p>
                </div>

                <button
                  type="button"
                  onClick={() => onFinish(createdGroup!, true)}
                  className="w-full py-4 bg-primary text-white font-bold rounded-xl text-lg shadow-xl shadow-emerald-200/50 dark:shadow-none hover:bg-primaryDark transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2"
                >
                  Montar Escala Agora
